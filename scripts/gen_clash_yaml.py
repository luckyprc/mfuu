...
def parse_ss(link: str):
    # 只做基础 ss 解析（method/pass 可能在 userinfo 或 base64 部分）
    u = urlsplit(link)
    server = u.hostname
    port = int(u.port or 0)
    ...
    # 常见格式：ss://method:pass@host:port
    if userinfo and password:
        cipher = userinfo
        pwd = password
    else:
        # 可能 ss://BASE64(method:pass)@host:port
        # 如果 username 存在但 password 为空，通常是 base64 后的 method:pass
        try:
...
def main():
    ...
    cfg = {
        ...
        "proxies": proxies,
        "proxy-groups": [
            {"name": "AUTO", "type": "url-test", "proxies": [p["name"] for p in proxies], "url": "http://www.gstatic.com/generate_204", "interval": 600},
